...
c $Id: fc.for 243 2008-10-29 10:10:19Z cct $
...
c               2: sum
c               3: add biases to ifile1 (one for each data)
c               4: merge file1 and file2 (both with 'ndata' points)
...
c
      write(*,1)
1     format(' *******************************************************',
     ./,     ' *    FC  -  GRAVSOFT file comparison  -  (c) RF/KMS   *',
     ./,     ' *******************************************************',
...
2     format(a128)
      write(*,12)
12    format(' input kind (0:stat, 1:dif, 2:sum, 3:bias, 4:mrg),',
     .' ndata, hist.sp.: ')
...
c
      ...
      lofile = (kind.gt.0.or.ldif.or.lbias.or.lmerge)
...
c
     ...
     .stop '*** ndata too big, sorry ***' 
      ...
      if (kind.ge.10) then
        ...
        lcol = .true.
        write(*,901)
901     format(' input total number and data positions: ')
...
c
      if (lbias) then
        write(*,902)
902     format(' input bias(es) to be added to data: ')
        read(*,*) (bias(j),j=1,ndata)
      endif
c
      ndata1 = ndata
      if (ltrsh) ndata1 = ndata+1
c
...
c
     ...
     .(rold(i),i=1,ndata1)
      ...
      lutm = (abs(rfiold).gt.100.or.abs(rlaold).gt.100)
      lgeo = (.not.lutm)
      if (lutm) write(*,19)
...
c
20    if (lfile2)
     .read(21,*,end=90) ist2,rfi2,rla2,rh2,(r(2,i),i=1,ndata)
203   if (lcol) then
...
201     r(1,j)=data(kk)
        ...
        if (kind.eq.5) then
          if (r(1,ndata1).gt.trsh) then
            nrej = nrej +1 
            goto 20
          endif
        endif
      endif
      if (ist1.ne.ist2.and.(.not.lbias)) then
        write(*,*)
...
c
      do 21 i=1, ndata
        if (kind.eq.1.or.kind.eq.5) then
          r(3,i) = r(1,i)-r(2,i)
...
91    format(/' --- fc statistics ---, number of points: ',i5)
      if (ldif) write(*,911)
...
92    format(' fc statistics, file3 = file1 - file2,',
     *' number of points: ',i5)
      if (kind.eq.2) write(*,93) n
93    format(' fc statistics, file3 = file1 + file2,',
     *' number of points: ',i5)
